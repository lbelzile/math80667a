...
pchisq(deviance(mod_null), df = mod_null$df.residual, lower.tail = FALSE)
```
...
BM04_T2 <- BM04_T2 |>
  dplyr::mutate(sym = gnm::Symm(black, white),
                pred = predict(mod_null, type = "response"))

knitr::kable(xtabs(count ~ black + white, data = BM04_T2))

xtabs(pred ~ black + white, data = BM04_T2) |>
  knitr::kable(booktabs = TRUE) |>
  kableExtra::kable_styling() |>
  kableExtra::cell_spec(c(1,9), bold = TRUE, background = 2)
  kable(booktabs = T) %>%
  kable_styling() %>%
