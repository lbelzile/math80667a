...
colnames(BM04_T2tab) <- c("no", "1W", "2W")
rownames(BM04_T2tab) <- c("no", "1B", "2B")
...
model_null <- glm(count ~ white*black, #saturated model 
             data = BM04_T2, 
             family = poisson) # default log-link
# Null model - same average for off diagonal entry
# Get back intger codes and create a factor with the same entries for 01 and 10, etc.
iwhite <- as.integer(BM04_T2$white) - 1L
iblack <- as.integer(BM04_T2$black) - 1L
BM04_T2$sym <- factor(paste0(pmin(iwhite, iblack), pmax(iwhite, iblack)))
model_alt <- glm(count ~ sym, #saturated model 
             data = BM04_T2, 
             family = poisson)
# Compare the two nested models
anova(model_alt, model_null,  test = "LRT")
pchisq(deviance(model_alt), df = model_alt$df.residual, lower.tail = FALSE)

# Pearson test - observed minus expected under model sum (O-E)^2/E
PearsonX2 <- sum(residuals(model_alt, type = "pearson")^2)
...
#| echo: false
THIS IS WRONG?
# Different approach, using contrasts
emmeans(model_null, specs = c("black", "white")) |>
  ...
  joint_tests()
model0 <- glm(count ~ white + black, #main effect model
             data = qty, 
             family = poisson) 
sum(residuals(model_alt, type = "pearson")^2)
...
# Compute chi-square test with sub-table
...
chisq.test(tabs)
# In the paper, all participants were included for this test 
# but other were excluded at a latter stage. 
# DA23_E2 only includes the latter (i.e., the subset)
```
