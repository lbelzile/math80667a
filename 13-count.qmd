...
In contingency tables, we can use Pearson's $\chi^2$ test to compare observed counts to postulated expected counts, comparing the statistic to a chi-square distribution. The large-sample approximation is adequate provided that the expected counts in each subgroup, $E_i$, are larger than 5.

The statistic is of the form
$$P =\sum_{i=1}^I \sum_{j=1}^J \frac{(O_{ij}-E_{ij})^2}{E_{ij}}$$
where $E_i$ is the expected count under the null hypothesis, and $O_i$ are the observed entries.
The degrees of freedom are $N$ (here $IJ=N$, the number of cells) minus the number of parameters under the null hypothesis.
