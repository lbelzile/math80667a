...
```{r infer-odds-ratio-permutation, cache = TRUE, eval = TRUE, echo = FALSE, fig.cap = "Histogram of the simulated null distribution of the odds ratio statistic obtained using a permutation test; the vertical red line indicates the sample odds ratio."}
library(infer)
...
LRMM22_S1  |>
  group_by(role) |>
  summarize(mean = mean(appreciation),
            sd = sd(appreciation),
            n = n())
broom::tidy(aov(appreciation~ role, data =
                  LRMM22_S1))
```
