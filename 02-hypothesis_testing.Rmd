...
- an estimator is the procedure or formula telling us how to transform the sample data into a numerical summary. Its output is random: even if you repeat a recipe, you won't get the same exact output every time.
...
\end{align*}
The square root of the variance of a statistic is termed **standard error**; it should not be confused with the standard deviation $\sigma$ of the population from which the sample observations $Y_1, \ldots$ are drawn. The standard error of the sample mean is $\mathsf{se}(\overline{Y}) = \sigma/\sqrt{n}$. Thus, the precision of the sample mean increases proportionally to the square root of the sample size: the standard error gets halved if we double the number of observations, but only decreases by a factor 10 if we have 100 times more observations. Both standard deviation and standard error are expressed in the same units as the measurements, so are easier to interpret than variance. 

Since the standard error is a function of the sample size, it is good practice to report instead the empirical standard deviation in reports.
...
For example, in completely randomized experiments with a single treatment with $k$ levels, we might be interested in testing whether the mean of $K$ different sub-populations are equal. Let $\mu_1, \ldots, \mu_K$ denote the expectation (theoretical mean) of each of the $K$ sub-populations. Equality of means translates into the hypotheses
\begin{align*}
\mathscr{H}_0:& \mu_1 = \cdots = \mu_K
\mathscr{H}_a:& \text{at least two means are different, }\mu_i \neq \mu_j (1 \leq i < j \leq K).
\end{align*}
Note that the null hypothesis is a single value with $K-1$ restrictions, whereas the alternative is the complement, i.e. all potential scenarios for which not all expectations are equal.
...
If there was no discrimination based on sex, we would expect the proportion of people promoted to be the same overall; this is $(32+19)/93 =$`r round((32+19)/93,2)` for the pooled sample. The chi-square test compares the expected counts (based on equal promotion rates) to the observed counts, suitably standardized: if the discrepancy is large, than this casts doubt on the validity of the null hypothesis.
...
## Same thing, using tidymodels
# Create data frame - categorical variables are factors
# rep replicates elements
dat_exper1 <- tibble::tibble(
  action = c(rep("promote", 32),
             rep("hold file", 12),
             rep("promote", 19),
             rep("hold file", 30)), 
  sex = c(rep("male", 44),
...
observed_statistic <- dat_exper1 %>%
  specify(action ~ sex, success = "promote") %>%
  calculate(stat = "Chisq", order = c("male", "female"), correction = FALSE)
chisq_test(ad_data, Genotype ~ Class,)
library(tidymodels) 

test <- chisq.test(x = rosen_exper1, correct = FALSE)
fisher.test(x = rosen_exper1)
tidy
```
