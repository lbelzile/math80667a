...
@fig-ancovaresid shows that, due to the strong correlation, the variability of the measurements is smaller on the right-hand panel (corresponding to the analysis of covariance model) than for the centred response on the left-hand panel.
...
#| eval: true
#| cache: true
#| fig-cap: "Response after subtracting mean (left) and after detrending and centering the mean (right)."
g3 <- ggplot(data = dat,
             aes(color = group,
                 x = group,
  ...
  labs(y = "centered response",
       subtitle = "ANOVA")+
  theme_classic()
dat$resid <- resid(lm(response ~ group + covariate,
         data = dat))
...
::: {#exm-leechoi-ancova #leechoi name="Inconsistency of product description and image in online retailing"}
