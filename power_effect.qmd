...
One common measure employed in software is Cohen's _f_ [@Cohen:1988], the square of which for a one-way ANOVA with factor $A$ (equal variance $\sigma^2$) with more than two groups, 
$$
...
\widehat{R}{}^2 = \frac{F\nu_1}{F\nu_1 + \nu_2}
$$
where for the one-way ANOVA $\nu_1 = K-1$ and $\nu_2 = n-K$ are the degrees of freedom of a design with $n$ observations and $K$ experimental conditions. From this, we can build an estimator of Cohen's $f$, 
where for the omnibus test of the one-way ANOVA, we get
$$
\widehat{f} = \sqrt{F\nu_1/\nu_2}
$$
