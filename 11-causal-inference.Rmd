...
#| eval: true
#| echo: false
#| tbl-cap: "Linear causal mediation analysis: parameter estimates and nonparametric bootstrap confidence intervals and $p$-values with the percentile method using $B=5000$ bootstrap samples."
x <- summary(linmed)
...
knitr::kable(smat, 
             digits = 3,caption = "Linear causal mediation analysis: parameter estimates and nonparametric bootstrap confidence intervals and $p$-values with the percentile method using $B=5000$ bootstrap samples.") |>
  kableExtra::kable_styling()
...
knitr::kable(tab_anova,
  ...
  caption = "Analysis of variance table for the linear moderation model.") |>
...
<!--
To fix ideas, we consider a Study 1 of @Roczniewska.Higgins:2019, who studied the impact of changes in the Polish education system, whereby middle school was due to disappear and be replaced with a new system with 8 years of elementary school and four years of high school. The authors surveyed teachers in middle and primary schools to see which would be the impact with two messages, either prevention or promotion.

We run the bootstrap manually.
-->
...
modM1 <- lm(fluency ~ regfocus*promem + regfocus*prevem,
   data = data, subset = indices)
modM2 <- lm(fairness ~ regfocus*promem + regfocus*prevem,
   data = data, subset = indices)
modM3 <- lm(engagement ~ regfocus*promem + regfocus*prevem,
   data = data, subset = indices)
modY <- lm(dopenness ~ regfocus + promem + prevem +
             regfocus:promem + regfocus:prevem +
             engagement + fairness + fluency +
             change + change:fairness,
   data = data, subset = indices)
# Stack all model parameters and return a vector
c(coef(modM1), coef(modM2), coef(modM3), coef(modY))
}
npbootstrap <- boot::boot(
  data = RH19_S1,
  statistic = boot_fn,
  stype = "i",
  R = 4999L)
...
coef <- npbootstrap$t0
confints <- apply(npbootstrap$t, 2, quantile, c(0.025, 0.975))
pvals <- sum(abs(air.boot$t[,2]-1) > abs(air.boot$t0[2]-1))/(1+air.boot$R)
